...
// 恢复所有动态 DOM（重新插入）- 基础版本
factory.resumeAll();

// 智能恢复所有动态 DOM（递归依赖解决版本）- 增强版本
await factory.resumeAllSmart();
...
### 智能恢复机制
新的 `resumeAllSmart()` 方法实现了智能递归恢复机制，能够处理复杂的依赖关系：

- **依赖检查**：自动检测元素与其父元素的依赖关系
- **递归恢复**：当子元素恢复失败时，自动尝试恢复其父元素
- **循环防护**：防止无限递归和死循环
- **失败处理**：对于无法恢复的元素，自动放弃并记录日志
